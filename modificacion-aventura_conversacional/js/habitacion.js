...
var habitacion={
    ...
    moverJugador(mov,jug,tramp){
        console.log("tu numero de la (mala) suerte es " + jug.suerte);
         
        switch (mov) {
            case "N":
                if(jug.y!=1){
                    if(jug.suerte==(jug.y-1)+ jug.x){
                        this.colocarElm(jug.x,jug.y,"-")
                        jug.y=jug.y-1;
                        this.colocarElm(jug.x,jug.y,"X");
                        console.log("TUVISTE MALA SUERTE, HAS CAIDO EN UNA TRAMPA");
                        break
                    }else if(jug.x==tramp.ejx && jug.y-1==tramp.ejy){
                        this.colocarElm(jug.x,jug.y,"-")
                        console.log("HAS MUERTO, TE PINCHASTE CON UN PINCHO");
                    }else{
            ...
            case "E":
                if(jug.x!=(this.anchoMapa-this.padIzq-this.padDrc-1)){
                    if(jug.suerte==jug.y+(jug.x+1)){
                        this.colocarElm(jug.x,jug.y,"-");
                        jug.x=jug.x+1;
                        this.colocarElm(jug.x,jug.y,"X");
                        console.log("TUVISTE MALA SUERTE, HAS CAIDO EN UNA TRAMPA");
                        break
                    }else if(jug.x+1==tramp.ejx && jug.y==tramp.ejy){
                        this.colocarElm(jug.x,jug.y,"-")
                        console.log("HAS MUERTO, TE PINCHASTE CON UN PINCHO");
                    }else{
